...
const BRANCH_X = 220;       // center → branch 横距離
const CHILD_X = 190;        // branch → children 横距離
const CHILD_V_SPACING = 46; // 子ノード縦間隔（ノード高 34 + 余白 12）
const BRANCH_GAP = 36;      // ブランチ間縦ギャップ

// ── カラーパレット（ブランチごとに循環） ────────
const PALETTE = [
  { fill: "#fef3c7", stroke: "#d97706", text: "#92400e" }, // amber
  { fill: "#dbeafe", stroke: "#3b82f6", text: "#1e3a8a" }, // blue
  { fill: "#d1fae5", stroke: "#10b981", text: "#065f46" }, // emerald
  { fill: "#ede9fe", stroke: "#8b5cf6", text: "#4c1d95" }, // purple
  { fill: "#ffe4e6", stroke: "#f43f5e", text: "#9f1239" }, // rose
];

// ── sessionStorage キー（AiGrill と共有） ────────
const SESSION_KEY = (id: number) => `aigrill-${id}`;

// ── SVGテキスト（長い場合は2行に折り返し） ───────
function SvgText({
  text,
  x,
  y,
  ...
  return (
    <text textAnchor="middle" fontSize={fontSize} fill={fill}>
      <tspan x={x} y={y - lh / 2} dominantBaseline="middle">
        {text.slice(0, mid)}
      </tspan>
      <tspan x={x} y={y + lh / 2} dominantBaseline="middle">
        {text.slice(mid)}
...
function computeLayout(data: MindMapData): BranchPos[] {
  ...
  function layoutGroup(
    ...
    // 各ブランチに必要な高さ = 子ノード数 × 縦間隔（最低 50px）
    const heights = branches.map((b) =>
      Math.max(50, b.children.length * CHILD_V_SPACING)
    );
    const totalH =
      heights.reduce((a, b) => a + b, 0) + (branches.length - 1) * BRANCH_GAP;
  ...
  return (
    ...
    >
      ...
      {layout.map((branch, bi) => {
        ...
        // branch端とchild端の中点を縦トランクのX位置にする
        const branchEdge =
          branch.side === "right" ? branch.x + 58 : branch.x - 58;
        const childEdge =
          branch.side === "right"
            ? branch.children[0].x - 53
            : branch.children[0].x + 53;
      ...
      {layout.map((branch) => {
        ...
        return branch.children.map((child, ci) => (
          <g key={`child-${branch.paletteIdx}-${ci}`}>
            <rect
              x={child.x - 53} y={child.y - 17}
              width={106} height={34} rx={10}
              fill={p.fill} stroke={p.stroke} strokeWidth={1.5}
            />
            <SvgText text={child.label} x={child.x} y={child.y}
              maxW={96} fontSize={11} fill={p.text} />
          </g>
        ));
      })}
      ...
      {layout.map((branch) => {
        ...
        return (
          <g key={`branch-${branch.paletteIdx}`}>
            <rect
              x={branch.x - 58} y={branch.y - 19}
              width={116} height={38} rx={12}
              fill={p.fill} stroke={p.stroke} strokeWidth={2}
            />
            <SvgText text={branch.label} x={branch.x} y={branch.y}
              maxW={106} fontSize={12} fill={p.text} />
          </g>
        );
      })}
